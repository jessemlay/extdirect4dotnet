...
{
    /// <summary>
    /// An Abstract Class that adds Paging or Grouping functionality to the Read Method of a CRUD action
    /// Set the 
    /// </summary>
    public abstract class  SimpleCRUDActionHybrid : SimpleCRUDWithSorting, IActionWithBeforeInvoke<Hashtable>
    {
        public PostProcessing postProcessingMethod;
        ...
        public new object afterMethodInvoke(DirectMethodType methodType, string methodName, object theReturnedObject)
        {
            if (methodType == DirectMethodType.Read)
            {
                ...
                if (postProcessingMethod == PostProcessing.Grouping)
                {
                ...
                else if (postProcessingMethod == PostProcessing.Paging)
                {
                    Hashtable storeResponse = (Hashtable)theReturnedObject;
                    Object returnedByTheReadMethod = storeResponse[this.getMetaData().getRootPropertyName()];
